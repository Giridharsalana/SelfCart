...
export const bulkUpdateInventory = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
...
export const generateInventoryReport = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
export const autoReorderProducts = functions.pubsub
